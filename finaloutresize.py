...
class LaneLineHistory:
    def __init__(self, queue_depth=2, test_points=[50, 100, 150, 250], poly_max_deviation_distance=50):
...
class AdvancedLaneDetectorWithMemory:
    ...
    def __init__(self, objpts, imgpts, psp_src, psp_dst, sliding_windows_per_line, 
                 ...
                 small_img_size=(80, 80), small_img_x_offset=5, small_img_y_offset=5,
                 img_dimensions=(253, 450), lane_width_px=320, 
                 lane_center_px_psp=225, real_world_lane_size_meters=(32, 3.7)):
        self.objpts = objpts
        self.imgpts = imgpts
    ...
    def draw_lane_curvature_text(self, img, left_curvature_meters, right_curvature_meters, center_offset_meters):
        ...
        cv2.putText(img, txt_values, (offset_x, offset_y + self.small_img_y_offset * 5), font, 1, (255,255,255), 1, cv2.LINE_AA)
    ...
    def draw_lane_lines(self, warped_img, undist_img,left_line, right_line):
        ...
        cv2.polylines(out_img, pts_left, False,  (0, 0,255), 5)
        cv2.polylines(out_img, pts_right, False, (0, 0,255), 5)
        
        for low_pt, high_pt in left_line.windows:
            cv2.rectangle(out_img, low_pt, high_pt, (0, 0, 255), 3)
        ...
        # Warp the blank back to original image space using inverse perspective matrix (Minv
         # Draw the lane onto the warped blank image
        cv2.polylines(color_warp1, pts_left, False,  (0, 0,255), 9)
        cv2.polylines(color_warp1, pts_right, False, (0, 0,255), 9)
        
        
        
        newwarp = cv2.warpPerspective(color_warp1, self.M_inv_psp, (undist_img.shape[1], undist_img.shape[0])) 
        # Combine the result with the original image
        result = cv2.addWeighted(undist_img, 1, newwarp, 0.6, 0)
...
#NVT1 without resize
#pts = np.array([[170,bottom_px],[550,530],[740,530], [870, bottom_px]], np.int32)
  
#NVT1 with resize
#pts = np.array([[100,bottom_px],[235,180],[290,180], [360, bottom_px]], np.int32) 

#nvt2 resized
# pts = np.array([[130,bottom_px],[205,190],[290,190], [360, bottom_px]], np.int32) 

#nvt4 resized
#pts = np.array([[120,bottom_px],[205,190],[270,190], [350, bottom_px]], np.int32)
 
#4k01 resized
#pts = np.array([[140,bottom_px],[190,190],[290,190], [340, bottom_px]], np.int32) 

#NVT5 resized
#pts = np.array([[60,bottom_px],[200,180],[260,180], [320, bottom_px]], np.int32) 

#videocurve1 /  trim2
#pts = np.array([[80,bottom_px],[155,130],[270,130], [360, bottom_px]], np.int32)
...
dst_pts = np.array([[100, bottom_px], [100, 0], [400, 0], [400, bottom_px]], np.float32)
'''
...
def inputImg():
    ...
    ld = AdvancedLaneDetectorWithMemory(opts, ipts, src_pts, dst_pts, 10, 20, 25)
...
def rplay():
    ...
    project_video_path = askopenfilename()
    project_video_output_path = 'output_videos/NVT1_outx.mp4'
    detector = AdvancedLaneDetectorWithMemory(opts, ipts, src_pts, dst_pts, 10, 50, 20)
...
b7  = Button(window, text="Area Marked \n Result", bg="grey", fg="cyan",font=("Times New Roman",15),command=finaloutput)
...
b11  = Button(window, text="Real-time \n Output", bg="grey", fg="cyan",font=("Times New Roman",15),command=vplay)
b11.place(x=210,y=690,anchor=W,height=60,width=170)
b12  = Button(window, text="Video \n Output", bg="grey", fg="cyan",font=("Times New Roman",15),command=rplay)
b12.place(x=210,y=620,anchor=W,height=60,width=170)
window.mainloop()
